...
#%% Intensity axis
fig, multi_ax = plt.subplots(nrows=2,sharex='col',figsize=[7.5,7.5],gridspec_kw={'height_ratios': [3,1]},tight_layout=True)
xlim_low = [0.8,12]
hwidth = 10 #headwidth
hlengths = 0.2 #head length - shorter in first plot

#colour dictionary
col = {'nebula':'black','dynamo':'black','uncertain':'#007aaf','nothing':'black'} #other angrite colour '#A00143'
cs = 5 #capsize
for i, met in enumerate(mclass):
    mdata = paleo2.loc[paleo2['Classification']==met,:] #filter by class
    mdata.reset_index(inplace=True)
    
    for axes in multi_ax:
