...
np.savez(f'Results_combined/Timestep_test/run_{run}_diff', Tdiff = Tdiff, Xfe = Xfe, Xsi = Xsi, cp = cp, Ra = Ra, Ra_crit = Ra_crit, convect = convect, d0=d0, t_diff = t_diff, H=H)
...
np.savez('Results_combined/Timestep_test/run_{}'.format(run), Tc = Tc, Tc_conv = Tc_conv, Tcmb = Tcmb,  Tm_mid = Tm_mid, Tm_conv = Tm_conv, Tm_surf = Tm_surf, 
...
with open('Results_combined/Timestep_test/run_info.csv','a') as f_object:         
...
with open('Results_combined/Timestep_test/timestep_test.csv','a') as f_object:
     writer_object = writer(f_object) #pass file object to csv.writer
