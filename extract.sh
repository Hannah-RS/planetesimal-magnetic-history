...
scp -r exet5460@gateway.arc.ox.ac.uk://data/eart-astroid-evo/exet5460/Asteroid_model/Results/$1 Results_combined/$1/ #copy from hpc
nfolders=$(ls Results_combined/$1|wc -l) #count number of folders
cp all_info.csv Results_combined/$1/all_sucess_info.csv #copy a template for final sucess results
cp all_info.csv Results_combined/$1/all_fail_info.csv #copy a template for final fail results
