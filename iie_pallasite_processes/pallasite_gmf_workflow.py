...
# -*- coding: utf-8 -*-
"""
Find possible parameters for pallasites from a set of model runs and test whether
Glorieta Mountain and Finmarken could have recorded a dynamo
"""
#%% Imports
import pandas as pd
import numpy as np
...
from pallasite_dynamo_functions import dynamo_check, paleo_check, dynamo_status
from pallasite_cooling_functions import find_temp_depth, check_623_cool, find_abs_depth
...
gmf_data = {'cr_yang_low': np.array([2.2, 17.5]),
...
pdata = pd.DataFrame(columns=['run','d1_low','d1_up','d2_low','d2_up','d3_low',
                                      ...
                                      'c2','c3','c4','c5','subfolder','gm_dlow',
                                      'gm_dup','f_dlow','f_dup','gm_tlow','gm_tup',
                                      'f_tlow','f_tup','gm_mag','f_mag',
                                      'gm_core_solid','f_core_solid'])
#%% Loop over runs
...
for run in mout['run']:
    ...
    #if all checks pass, check Glorieta Mountain and Finmarken
    if f3 == True:
        #find contour for Yang et. al. 2010 data, find depths where cooling rates match
        rindgm = find_temp_depth(925,gmf_data['cr_yang_low'],gmf_data['cr_yang_up'], t, temp, tempdt) 
        #find depths of each sample
        depthgm, timegm = find_abs_depth(rindgm,rprof,t,temp,925)
    ...
    pdata.loc[i] = [run, depth[0, 0], depth[0, 1], depth[1, 0], depth[1, 1], depth[2, 0], depth[2, 1],
                ...
                depthgm[0, 0], depthgm[0, 1], depthgm[1, 0], depthgm[1, 1], timegm[0,0], timegm[0,1], timegm[1,0], 
                timegm[1,1], Bmodelgm[0], Bmodelgm[1],cgm[0],cgm[1]]
    i += 1
