...
def thermal_evolution(tstart,tend,dt,T0,f0,sparse_mat_c,sparse_mat_m):
    ...
    if Ra[0] < Racrit[0]:# not convecting
        ...
        Tm_conv[0] = T0_mantle[1] + dTmdt_calc(tsolve[0],T0_mantle[1],Fs[0],Fcmb[0])*dt #take temp one above CMB
    ...
    for i in range(1,m):
        ...
        if Ra[i] <= Racrit[i] or cond==1: #once Rayleigh number subcritical don't want to use that criterion anymore
            ...
            Tm_conv[i] = Tm_old + dTmdt_calc(tsolve[i-1],Tm_old,Fs[i-1],Fcmb[i-1])*dt #temperature of convecting region 
