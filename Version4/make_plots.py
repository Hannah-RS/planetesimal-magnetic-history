...
plt.legend()
#plt.savefig('Plots/Tflux_run{}.png'.format(run))
...
#plt.savefig('Plots/Remf_run{}.png'.format(run))
...
#lid thickness
plt.figure()
plt.loglog(t_plot[:cond_i],2*d0[:cond_i]/r)
...
t_plot2 = np.arange(1,10,0.01)
plt.figure()
for i in range(1,2):
    plt.plot(t_plot2,T_profile[:,i],label='{} km'.format(0.5*i))
plt.xlabel('time')
plt.ylabel('T/K')
#plt.xlim([2.5,10])
plt.legend()
